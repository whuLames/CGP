...
__global__ void expand_shared_node_kernel(
  ...
  for (std::size_t i = blockIdx.x; i < unique_count; i += gridDim.x) { // assign one block for one vertex
    ...
    for (auto edge = begin + threadIdx.x; edge < end; edge += blockDim.x) {
      ...
      if constexpr (std::is_same<Policy, algorithms::bfs_policy>::value) {  // The fixed & optimized implementation for BFS
        ...
        while (bits != 0) {
          ...
          bits &= (bits - 1); // remove the least significant bit
        }
      } else {
        /*
        ...
        while (bits != 0) {
          ...
          if (query_id < query_count) {
            ...
            if (source_distance != Policy::infinity()) {
              ...
              However, if we maintain a copy for the query-vertex data, here we can avoid atomic operator.
              But, for the update, we still need the atomic operator.
              */
  ...
  for (std::size_t i = blockIdx.x; i < unique_count; i += gridDim.x) {
    ...
    for (auto edge = begin + warp_in_block; edge < end;
      ...
      if constexpr (std::is_same<Policy, algorithms::bfs_policy>::value) {
        ...
        improved = static_cast<query_mask_t>(__shfl_sync(0xffffffffU, reduced, 0));
      }
...
void launch_fused_pull(graph_t graph,
  ...
  if (query_count <= 32) {
    ...
    throw std::invalid_argument(
        "fused pull supports at most 64 queries");
  }
}
      ...
      if (use_pull && shared_push) {
        ...
        if (options.pull_strategy == pull_strategy_t::ge_spmm) {
          auto launch_fused_pull = [&]() {
            detail::launch_fused_pull<Policy, graph_t, vertex_type>(
                graph, query_count,
                thrust::raw_pointer_cast(values.data()),
                thrust::raw_pointer_cast(visited_mask.data()),
                thrust::raw_pointer_cast(next_frontier_mask.data()),
          ...
          profile.compact_ms += detail::timed_gpu(
              stream, options.profile_iterations, [&]() {
                ...
                thrust::inclusive_scan(
                    ...
                    shared_virtual_degrees.begin());
                detail::compact_shared_pull_frontier_kernel<vertex_type>
                    <<<detail::grid_for(vertex_count, threads), threads, 0,
                       stream>>>(
                        thrust::raw_pointer_cast(next_frontier_mask.data()),
                        vertex_count,
