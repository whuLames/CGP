...
struct sswp_policy {
  ...
  static constexpr algo_kind_t algorithm_kind = algo_kind_t::sswp;
  ...
  static constexpr value_type source_value() {
    return std::numeric_limits<value_type>::infinity();
  }

  template <typename weight_t>
  static constexpr value_type relax(value_type source_width, weight_t weight) {
    const value_type capacity = static_cast<value_type>(weight);
    return source_width < capacity ? source_width : capacity;
  }
