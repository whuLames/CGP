...
using puercgp::algorithms::dispatch_candidate_pull;
...
// 测试 dispatcher candidate（push + pull 三种 algo_kind）
__global__ void test_candidate(unified_value_t* out_push,
                               unified_value_t* out_pull) {
  if (threadIdx.x == 0) {
    // push 模式
    out_push[0] =
        dispatch_candidate_push(algo_kind_t::bfs, 0.0f, 1.0f, 3);  // 0+1=1
    out_push[1] = dispatch_candidate_push(
